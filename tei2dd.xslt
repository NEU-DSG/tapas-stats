...
<xsl:stylesheet 
  ...
  <d:doc>
    <d:desc>
      ...
      <d:p type="updated">2015-10-28 by Syd for TAPAS -- 
        <d:ul>
          <d:li>fix bugs I introduced 2015-10-20 or so</d:li>
          <d:li>change labels for lists of element children and attr values</d:li>
          <d:li>list attribute values 1 per line rather than all in 1 line</d:li>
          <d:li>sor"degt the lists of elements, attrs, and values</d:li>
        </d:ul> and
      .</d:p>
      <d:p type="updated">2015-10-19 by Syd for use in TAPAS -- basically
      <d:ul>
        <d:li>presume no data dictionary available, so make default output
        header more robust for our purposes</d:li>
        <d:li>output only a <tt>&lt;div></tt> rather than whole XHTML file, unless
        asked (via new <tt>fullHTML</tt> parameter)</d:li>
      </d:ul></d:p>
  ...
  <xsl:param name="teiOutPath" select="concat('/tmp/data_dictionary_',replace(tokenize( document-uri(/),'/')[last()],'\.[^.]+$',''),'.xml')"/>
  <xsl:param name="P5source" select="'p5subset.xml'"/>
  <xsl:param name="dictFile">
    <xsl:if test="doc-available('sample_dictionary.xml') and false()">
      <xsl:value-of select="'sample_dictionary.xml'"/>
    </xsl:if>
  </xsl:param>

  <xsl:variable name="dictAvail" select="'no'"/>
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="inputPATH" select="document-uri(/)"/>
    <xsl:variable name="inputFN" select="tokenize($inputPATH,'/')[last()]"/>
    <xsl:variable name="intermediate_TEI">
      <xsl:variable name="teiFile" select="document($P5source)"/>
      <TEI xmlns="http://www.tei-c.org/ns/1.0">
        <xsl:choose>
          <xsl:when test="$dictAvail = 'yes'">
            <xsl:value-of select="concat('debug01: ',$dictAvail, '=', $dictFile)"/>
          ...
          <xsl:otherwise>
            <teiHeader>
              <fileDesc>
                <titleStmt>
                  <title type="main">XML Usage</title>
                  <title type="debug">
                    <xsl:copy-of select="/TEI/teiHeader/fileDesc/titleStmt/title[1]"/>
                  </title>
                </titleStmt>
                <publicationStmt>
                ...
                <sourceDesc>
                  <p>TAPAS: Summary of XML constructs in TEI file
                    <ref type="targetTitle" target="{$inputPATH}">
                      <xsl:value-of select="$inputFN"/>  
                    </ref>.</p>
                </sourceDesc>
              </fileDesc>
            </teiHeader>
                ...
                <xsl:if test="position() = 1">
                  ...
                  <div>
                    ...
                    <xsl:if test="$attMayContain != ''">
                      <ab>
                        <xsl:attribute name="type" select="'values'"/>
                        <xsl:for-each select="tokenize($attMayContain,'\s+')">
                          <xsl:sort/>
    ...
    <xsl:if test="$outputFormat = 'tei'">
      <xsl:result-document href="{$teiOutPath}">
        <xsl:apply-templates select="$intermediate_TEI" mode="copy"/>
      </xsl:result-document>
    </xsl:if>
  ...
  <xsl:template match="/" mode="TEI2HTML">
    <xsl:if test="$debug eq 'true'">
      <xsl:message>debug02: I'm in / [TEI2HTML]</xsl:message>
    </xsl:if>
    <html xmlns="http://www.w3.org/1999/xhtml">
      <head>
        ...
        <style type="text/css">
          ...
          /* the FN link (or others in top banner area) */
          div.hero a {color: white; }
          /* unvisited link */
          a:link {
          ...
          /* Styles for 4 boxes under each element */
          ...
          .children, .values {overflow: auto; margin-bottom: 15px;}
      ...
      <body>
        <div class="hero">
          <div class="tagline">
            ...
            <xsl:apply-templates select="//sourceDesc/p[1]"/>
            <br/>
  ...
  <xsl:template match="div" mode="TEI2HTML">
    <div class="entry">
      ...
      <div class="context">
        ...
        <xsl:apply-templates select="ab[@type = 'values']" mode="TEI2HTML"/>
      </div>
  ...
  <xsl:template match="ab[@type = ('parents','children','values')]" mode="TEI2HTML">
    <div>
      <xsl:attribute name="class" select="@type"/>
      <span class="label">
        <xsl:choose>
          <xsl:when test="@type eq 'parents'">
            <xsl:text>Contained by: </xsl:text>
          </xsl:when>
          <xsl:when test="@type eq 'children'">
            <xsl:text>Children: </xsl:text>
          </xsl:when>
          <xsl:when test="@type eq 'values'">
      ...
      <ul>
        <xsl:choose>
          <xsl:when test="(@type = 'values') and (parent::div[@type = 'attribute'])">
  ...
  <xsl:template match="ref[@type eq 'targetTitle']">
